...
"ai": {
  ...
  "gemini": {
    ...
    "model": "gemini-3.8-flash"  // オプション、デフォルトはgemini-3.8-flash
  }
}
```
...
- **危険な操作の警告**: DROP、DELETE、TRUNCATE、ALTERは、モデルが警告を付けたかどうかに関わらず、CQLの上に警告を表示します
- **確認が必要**: 目の前のクエリでEnterを押すまで、何も実行されません
- **実行前のプレビュー**: CQLは実行される形のまま表示され、先に編集できます
