...
func TestRuntimeChanges_AddConfirmQueries(t *testing.T) {
	...
	cmd := startCQLAIWithMCP(t, "testdata/dba.json") // DBA without confirmations
	defer stopCQLAI(cmd)
	time.Sleep(2 * time.Second)
...
func TestRuntimeChanges_DisableConfirmations(t *testing.T) {
	...
	cmd := startCQLAIWithMCP(t, "testdata/dba_confirm_all.json") // Starts with ALL
	defer stopCQLAI(cmd)
	time.Sleep(2 * time.Second)
...
func TestRuntimeChanges_PresetToFineGrained(t *testing.T) {
	...
	cmd := startCQLAIWithMCP(t, "testdata/readonly.json") // Preset mode
	defer stopCQLAI(cmd)
	time.Sleep(2 * time.Second)
