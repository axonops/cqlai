...
func TestRuntimePermissionConfigChanges(t *testing.T) {
	...
	t.Run("step2_change_to_readwrite", func(t *testing.T) {
		resp := callTool(t, "update_mcp_permissions", map[string]any{
			"mode":           "readwrite",
			"user_confirmed": true,
		})
...
func TestPermissionLockdownEnforcement(t *testing.T) {
	...
	cmd := startCQLAIWithMCP(t, "testdata/dba_locked.json")
	defer stopCQLAI(cmd)
	time.Sleep(2 * time.Second)
...
func TestConfirmationLifecycle(t *testing.T) {
	...
	t.Run("get_approved_empty", func(t *testing.T) {
		resp := callTool(t, "get_approved_confirmations", map[string]any{})
		if resp != nil {
	...
	t.Run("get_denied_empty", func(t *testing.T) {
		resp := callTool(t, "get_denied_confirmations", map[string]any{})
		if resp != nil {
	...
	t.Run("get_cancelled_empty", func(t *testing.T) {
		resp := callTool(t, "get_cancelled_confirmations", map[string]any{})
		if resp != nil {
