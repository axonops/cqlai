...
- **設定:**
    - 現在のディレクトリの`cqlai.json`、または`cqlshrc`と同じ場所の`~/.cassandra/cqlai.json`による簡単な設定。
...
| `Ctrl+Q` | 終了(確認あり)。`FILE > QUIT` と同じ | `Ctrl+Q`(`⌘+Q` はターミナル自体が受け取ります) |
...
タブは左から右に並び、キーもその順に並びます。

| ショートカット | アクション |
|----------|--------|
| `F2` | コンソール(入力したコマンドと結果の記録) |
| `F3` | スキーマ(クラスタのキースペースとテーブル、およびその定義) |
| `F4` | 結果(直前のクエリの出力、`OUTPUT`の形式で表示) |
| `F5` | トレース(`RESULTS` 内のもう一つのタブ。トレース有効時) |
| `F6` | Chat(AI会話) |

#### FILEメニュー

タブ行の `FILE` (Alt+F)を開くと、ファイルに関する操作がまとまっています。
...
`CONNECT` は接続先を尋ねます。ウィンドウは左右に分かれ、左側に保存済みの接続一覧
(その上に `+ Create New Connection`)、右側に選択中の接続の設定(ホスト、ポート、
キースペース、ユーザー名、パスワード、2つのタイムアウト、SSL設定)が並びます。
`←` `→` で両者を行き来します。

接続には名前があり、一覧にはその名前が表示されます。空のままならホスト名が使われ
ます。一覧の先頭がCQLAIの起動時に使う接続で `(default)` と表示されます。現在接続中
のものには `(connected)` と表示され、ウィンドウはその接続を開いた状態で始まります。

ファイル先頭の接続設定は、デフォルト接続を書き出したものです(手で `cqlai.json` を
読むものがホストを見つけられるように)。決めるのは一覧のほうで、変更すると先頭も
それに従います。`Save and Connect` は `cqlai.json` の `connections` に保存し、次回起動時の
接続先にします。`Connect` は保存せずに接続します。接続に失敗した場合は理由を表示
したままウィンドウが残るので、項目を直して再試行できます。
...
実行、`COPY TO`/`COPY FROM` はテーブル全体を出し入れします。1本目の線の下の
`PREFERENCES` は起動時の設定を編集し、2本目の下の `QUIT` は確認したうえで終了
します。

パスを尋ねるウィンドウ(`SAVE RESULTS`、`AUTOSAVE`、および3つのフォームの `File`
欄)は、ホームディレクトリのファイルブラウザを開いた状態で始まります。Tabで補完、
`..` で1つ上へ移動、ホイールで一覧をスクロールできます。

#### スキーマブラウザ

タブ行の `SCHEMA` (F3)は、クラスタのキースペースとテーブルをツリーとして左側に、
選択したものの定義を右側に表示します。キースペースをクリックすると定義が表示され、
展開されます。もう一度クリックすると折りたたまれます。テーブルをクリックすると
その `CREATE TABLE` が表示されます。

矢印キーでツリーを移動し(右で展開、左で折りたたみ)、ホイールはポインタのある側の
ペインをスクロールします。`Alt+↑`/`Alt+↓` は定義をスクロールします。それ以外の
キーはプロンプトに送られるので、テーブルの定義を見ながらクエリを入力できます。
`F3` をもう一度押すと、スキーマをクラスタから取得し直します。

タブを開くまで何も取得しません。最初に使うときにキースペース、最初に展開したときに
そのテーブル、最初に表示するときに定義を取得し、以降は保持します。

スキーマの変更は、どこで行われたかにかかわらず検知します(CQLAI内、cqlsh、別の
CQLAI、アプリケーション)。Cassandraはスキーマが変わるたびに変化するスキーマ
バージョンを保持しており、これは `nodetool describe cluster` の「Schema
versions」に表示される値です。CQLAIは5秒ごとにこの1行を読み、前回と違っていれば
スキーマブラウザとタブ補完の両方を取得し直します。ツリーを見ている最中であれば
その場で、そうでなければ次にタブを開いたときに更新されます。`F3` を押せばその場で
取得します。
...
| `↑`/`↓` | テーブルビューとトレースビューでは結果をスクロール。通常ビューではコマンド履歴をたどる。`Ctrl+P`はどのビューからでも履歴を呼び出す | 同じ |
...
#### マウスサポート
cqlaiはマウスボタンを端末に任せます。そのためテキスト選択、右クリック貼り付け、
ミドルクリック貼り付けは、他のプログラムとまったく同じように動作します。修飾キー
は不要です。

ホイールは代替スクロールモード経由で受け取ります。このモードでは端末がホイールの
回転を`↑`/`↓`のキー入力に変換します。つまりホイールは`↑`と`↓`がスクロールするもの
をスクロールします。テーブルビューでは結果、トレースビューではトレース、AIビュー
では会話です。

| アクション | 機能 |
|--------|----------|
| マウスホイール | 自動データロード付きで垂直スクロール |
| クリック+ドラッグ | テキスト選択(端末自身の選択機能) |
| 右クリック | 貼り付け(端末がそう割り当てている場合) |
| ミドルクリック | 選択バッファから貼り付け(Linux/Unix) |

幅の広いテーブルを水平にスクロールするには`Alt+←`/`Alt+→`、またはナビゲーション
モードで`<`と`>`を使います。ホイールでの水平スクロールは利用できません。端末は
垂直方向の回転のみをキー入力として通知するためです。
...
2. **CQLAI JSON設定ファイル**
   ...
   - `~/.cassandra/cqlai.json`(`cqlshrc`と同じ場所。新規作成時はここ)
   - `~/.cqlai.json`(ユーザーホームディレクトリ。従来の場所)
...
タブ行の `FILE > PREFERENCES` を選ぶと、CQLAI自身の設定項目
(結果の取得と表示、履歴ファイル、AIプロバイダ、認証プロバイダ)を
編集するウィンドウが開きます。入力して `Save` を押すと、読み込み元のJSONファイル、
どれも見つからなかった場合は `~/.cassandra/cqlai.json` に書き込みます。CQLAIが認識しない
キーがファイルにある場合はそのまま残されます。

`CHAT` にあるのは「どのプロバイダに尋ねるか」の一つだけです。APIキー、モデル、
接続先はプロバイダごとのものなので、その下の `CHAT - ANTHROPIC`、`CHAT - OLLAMA`
といったプロバイダ自身のセクションにあります。プロバイダを切り替えても入力し直す
必要はなく、誰のものか分からない欄にキーを入れることもありません。

接続先とその方法(ホスト、ポート、キースペース、認証情報、タイムアウト、SSL)は
CQLAIではなく接続に属するものなので、`FILE > CONNECT` で、その接続に対して
編集します。同じ設定を二つのウィンドウが書き込むのは、このプロジェクトが自らの
コードで繰り返し見つけてきた誤りです。

このウィンドウが編集するのは次回起動時の設定です。実行中のセッションは変更しません。
整合性レベル、ページサイズ、出力形式は、ステータス行または `CONSISTENCY`、
`PAGING`、`OUTPUT` で変更します。
...
2. `~/.cassandra/cqlai.json`(`cqlshrc`と同じ場所)
3. `~/.cqlai.json`(ユーザーホームディレクトリ)
4. `~/.config/cqlai/config.json`(Linux/macOSのXDG設定ディレクトリ)
